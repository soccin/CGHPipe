##
## $Source: /home/socci/lWork/CGHPipe/Rlib/agilentNorm.R $
## $Date: 2010/03/29 22:55:14 $
##

#Sys.setenv("R_GSCMD"="/home/liang/Work/CGHPipe/Rlib/RGS")
if(!exists("revision")) revision=list()
revision$agilentNorm = '$Id: agilentNorm.R 6f6fea87f1cb 2010/03/29 22:55:14 socci $'
...
dprint <- function(name, klass, plotfun, ...) {
  ...
  bitmap(fn,width=8.5,height=11,res=150)
...
agilentNorm <- function(fname, do.GC=TRUE, verbose=TRUE,
                        do.plot=TRUE, write.gpr=FALSE, default.grid=NULL) {
  ...
  ##
  ## Get design file (and map file) name from the
  ## Barcode field, and not the DesignFile file
  ## since the DesignFile gets messed up on REGRIDDED
  ## samples
  ##  
  fields=strsplit(header[2],"\t")[[1]]
  barcodefield=which(fields=="FeatureExtractor_Barcode")
  ##cat("=====================================================================\n")
  ##cat("barcodeFiles=",barcodefield,"\n")
  ##cat("fields=",fields,"\n")
  ##cat("=====================================================================\n")
  temp=strsplit(header[3],"\t")[[1]]
  barcodefile=temp[barcodefield]
  if(len(grep("^[0-9][0-9][0-9][0-9][0-9][0-9][0-9]",barcodefile))==0) {
     #InValid Barcode
     #Get Barcode from name of tif file
     pos=regexpr("MSK_",temp[barcodefield+1])
     barcodefile=substr(temp[barcodefield+1],pos+4,pos+19)
  }  
  ##
  ## Barcode [252152920842_S01]
  ##            ^^^^^--designCode
  ##
  root=substr(barcodefile,3,7)
  ...
  if (!exists("map")) { ## do this part once in the beginning
    ...
    if (file.exists(mapfile)) {
      ...
      print(dir("Maps"))
      if(!is.null(default.grid)) {
        cat("\n\n LOADING DEFAULT GRID",default.grid,"\n")
        load(default.grid)
      } else {
        stop()
      }
  ...
  if (write.gpr) {
   ##
   ## format = 1 for old (CGH analytics)
   ##        = 2 for new partek
   gpr.format=1 
   if(verbose) {cat("Writing GPR File ...")}
    if(len(header)!=9) {
      cat("\nINVALID HEADER LEN",len(header),"\n")
      stop("INVALID HEADER")
    }
    dii=which(!is.na(di))
    ...
    if(gpr.format==2) {
        ##
        ## Mode center leave log base 2
        ##
        dxx=density(d$R.norm,na.rm=T)
        ix=which.max(dxx$y)
        dd$LogRatio[di[dii]]=d$R.norm-dxx$x[ix]

        ##
        ## Get Rid of NA's
        ## 
        dd=dd[!is.na(dd$LogRatio),]

        newfile=paste(nameTmpl,"__GCNormV4P.txt",sep="")
    }
    ...
    if(gpr.format==2) {
      ##
      ## Get Rid of Useless columns only for PARTEK
      ##
      iCol=which(colnames(dd)=="rProcessedSignal")
      header[9]=paste(strsplit(header[9],"\t")[[1]][1:iCol],collapse="\t")
      dd=dd[,1:iCol]
    }
    for(line in header) {
      cat(line,"\n",file=ff,sep="")
